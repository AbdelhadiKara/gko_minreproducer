...
int main(int argc, char **argv) {
  ...
  std::shared_ptr const gko_exec = gko::HipExecutor::
  ...
  Kokkos::DefaultExecutionSpace> values_view("values_view", values_layout);
  ...
  values_view.data()); auto gko_idx=gko::array<int> ::view (gko_exec,
  idx_view.span(), idx_view.data());
       std::shared_ptr<gko::batch::matrix::Ell<double, int> >  batch_matrix_ell
  = gko::share(gko::batch::matrix::Ell<double>::create( gko_exec,
  ...
  logger
    ...
    //------------------------------------------------------------------------------------------
          Kokkos::View<double***, Kokkos::LayoutRight,
  Kokkos::DefaultExecutionSpace> x_view("x_view", batch_size, mat_size, 1);
          Kokkos::View<double**, Kokkos::LayoutRight,
  Kokkos::DefaultExecutionSpace> res_view("res_view", batch_size, mat_size);
  ...
  gko::dim<2>(x_view.extent(1), x_view.extent(2))),
                             gko::array<double>::view(gko_exec, x_view.span(),
  x_view.data()))); auto b=gko::share( gko::batch::MultiVector<double>::
                      create(gko_exec,
                             gko::batch_dim<
                                     2>(res_view.extent(0),
